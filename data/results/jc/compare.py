...
def get_stats(log_mass,conc):
    ...
    bins = np.arange(0,np.amax(log_mass)+0.5,0.25)
...
mcmc = np.genfromtxt(sys.argv[1],delimiter=',')
vel = np.genfromtxt(sys.argv[2],delimiter=',')
dens = np.genfromtxt(sys.argv[3],delimiter=',')

index = [int(x) for x in np.argwhere(dens[:,1] > 100)]
...
ax1.legend(loc=4, borderaxespad=0.5)
ax1.set_xscale('log')
ax1.set_xlim([4E13,2E15])
ax1.set_ylim([0,10])
...
ax1.set_xlabel("$\mathrm{Concentration\ (Our\ Method)}$",fontsize=25)
ax1.set_ylabel("$\mathrm{Concentration\ (Velocity\ Method)}$",fontsize=25)
ax1.set_xscale('log')
ax1.set_yscale('log')
...
ax2.set_xlim(xmin=np.amin(c_mcmc),xmax=np.amax(c_mcmc))
ax2.set_ylim(ymin=np.amin(c_mcmc),ymax=np.amax(c_mcmc))
