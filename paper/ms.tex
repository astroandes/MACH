...
the mass-concentration-redshift relation of dark matter halos. A
recent example is \citep{Ludlow2016}
...
A recent example of this methodology in cosmological simulation is
given by \cite{Klypin2016}.
...
In our bootstrapping experiments we take halos with at least
$10^{5}$ particles and subsample them by factors of $2$ up to
$10^{3}$ to measure the concentration in every resampling.
We use a two-sample Kolmogorov-Smirnov (KS) test to check that the
bootstrapped sets are a fair subsample of its parent.
Using the KS test we compare the list of radial distances from each
subsample against that of its parent halo.
We find that the resulting p-value distribution is flat.
This confirms that the radial distribution of particles in the
bootraped halo is consistent with coming from the distribution given
by the parent halo. 
...
How does this impact the most recent mass-concentration estimates? 
\cite{Ludlow2016} and \cite{Klypin2016} estimated the mass-concentration
relation over a suite of cosmological N-body simulations using the
density and velocity methods, respectively. 
Both used halos that had at least $5\times 10^{3}$ particles,
so their results should not be affected by the bias we report here.
However, Figure 16 in \cite{Klypin2016} seems to push the results
below this particle limit for redshifts larger than $z=2.5$ where
their simulations should not be able to reach the mass range
$5\times 10^{10}$\hMsun - $5\times 10^{12}$\hMsun that they claim. 
Consistent with this limitation \cite{Ludlow2016} only report results
up to a redshift of $z=3$.  
...
\vspace{0.1cm}
 ...
 Antioquia''. The authors also thank Tom\'as Verdugo, Stefan
 Gottloeber and Nelson Padilla for their comments on earlier versions
 of this work.
