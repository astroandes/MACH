...
\begin{abstract}
We present a jacknife method to estimate the bias on the estimated
concentration values as a function of particle number in the halo. 
We find that algorithms based on the maximum radial velocity
and radial particle binning show similar biases,
overestimating concentrations by a factor of $15\%-20\%$ for
halos sampled with $200$ particles.  
To overcome this bias we propose a new concentration estimation
algorithm that uses the integrated mass profile.
...
limitation for other methods, reducing the bias to $2\%-3\%$ for halos
sampled with $200$ particles. 
To keep the biases of the velocity and density methods to the same
low levels, dark matter halos should be sampled with at least $\sim
1000$ particles.
We also find that the mass-concentration relationship could be
shallower than expected from numerical experiments that do not take
into account this bias. 
From these results we believe that jacknife downsampling and the
concentration estimate based on the integral mass densities are
promising tools to probe the internal structure of dark matter halos
...
\begin{figure*}
\begin{center}
  \includegraphics[width=0.48\textwidth]{avg_foff_bolshoi.pdf}
  \includegraphics[width=0.48\textwidth]{avg_foff_vialactea.pdf}
\end{center}
\vspace{-0.5cm}
