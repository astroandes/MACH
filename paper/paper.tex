...
dark matter halos based on the radially integrated mass profile
...
resolution becomes a limitation for other methods.  Additionally, the
...
while density profile methods fail at the $100\%$ level.
...
We now review the key assumptions of the different methods used
to estimate the concentration before presenting our proposal.
...
Eq. (\ref{eq:profile}).
To build it from N-body data we follow the next steps.
